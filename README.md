...
| `hanko stamp [format]`               | Apply declared `stamp-targets:`. With `[format]`, only targets whose `format:` matches (`hanko stamp helm`, `hanko stamp nix`, …). |
| `hanko version go-ldflags`           | Emit `-X main.version=… -X main.commit=… -X main.date=…` for `go build` |
| `hanko version docker tags <image>`  | Fan version out into `<image>:<full>`, `:<major.minor>`, `:<major>`, `:latest`, `:<branch>-<sha>` |
| `hanko version docker labels`        | Emit `org.opencontainers.image.*` labels as `--label` args              |
